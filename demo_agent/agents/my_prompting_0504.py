...
class MyMainPrompt(PromptElement): 
    def __init__(self, 
        ...
        if len(obs_history) == len(states) + 1 and len(states) == len(actions): 
            ...
            # first forward dynamics, use obs + state
            self.obs = self.get_observation_state(obs_history[-1], states[-1])
        elif len(obs_history) < len(states) and len(states) == len(action): 
            # other forward dynamics, use state
...
"""
    ...
    def get_observation_state_action(self, obs, state, action): 
        observation_state = self.get_observation_state(obs, state)

        return observation_state + f"\n## Proposed Action:\n{action}\n\n"
...
answer:
<state>
Summarize the observation of the current step. Pay attention to the details of the UI
elements. Describe the effect that your previous action had, as well as elements you
can interact with on the current page.
</state>\
"""
...
Make sure to follow the template with proper tags:
<next_state>
My action entered text "quote" into the textbox 123, which leads to a new list of text items
appearing below textbox 123. All the items start with "quote", indicating the list may be
an autocomplete feature. The page shows:
