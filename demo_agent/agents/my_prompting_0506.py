...
class MyMainPrompt(PromptElement): 
    def __init__(self, 
        ...
        if len(obs_history) == len(states) + 1 and len(states) == len(actions): 
            # encoding, use just the obs
            self.obs = self.get_observation(obs_history[-1])
        elif len(obs_history) == len(states) and len(states) == len(actions) + 1: 
            # policy, use obs + state
            self.obs = self.get_observation_state(obs_history[-1], states[-1])
        elif len(obs_history) == len(states) and len(states) == len(actions): 
            # first forward dynamics, use obs + state + action
            self.obs = self.get_observation_state_action(obs_history[-1], states[-1], actions[-1])
        elif len(obs_history) < len(states) and len(states) == len(actions): 
            # other forward dynamics, use state + action
            # action value, use state + action
            self.obs = self.get_state_action(states[-1], actions[-1])
        elif len(obs_history) < len(states) and len(states) == len(actions) + 1: 
            # critic, use state # no, because it's too heavy weight
            # rollout policy, use state
            self.obs = self.get_state(states[-1])

        # self.obs = self.get_observation(obs_history[-1])
        # if len(obs_history) == len(states): # 
        #     if len(obs_history) == len(actions) + 1:
        #         self.obs_state = self.get_observation_state(obs_history[-1], states[-1])
        #     elif len(obs_history) == len(actions): 
        #         self.obs_state_action = self.get_observation_state_action(obs_history[-1], states[-1], actions[-1])
...
"""
    ...
    def get_history(self, obs_history, states, actions): 
        ...
        # for i in range(max(1, len(states) - 1), len(states)):
            # history_step = self.get_history_step(obs_history[i-1],
            #                                      states[i-1],
            #                                      actions[i-1])
            history_step = self.get_history_step(None,
                                                 states[i-1],
                                                 actions[i-1])
    ...
    def get_observation_state(self, obs, state): 
        return self.get_observation(obs) + f"\n## Inferred State:\n{state}\n"

    def get_observation_state_action(self, obs, state, action): 
        return self.get_observation_state(obs, state) + f"\n## Proposed Action:\n{action}\n"
    ...
    def add_screenshot(self, prompt):
        if isinstance(prompt, str):
            prompt = [{"type": "text", "text": prompt}]
        img_url = image_to_jpg_base64_url(self.screenshot)
        prompt.append({"type": "image_url", "image_url": img_url})
...
<strategy>
The next step will be to explore different ways to fill the form, such as clicking
its elements to explore options or filling parts of it with text. 
</strategy>\
...
# """
        ...
        history_steps = []
        for i in range(max(1, len(states) - 1), len(states)):
            history_step = self.get_history_step(self.obs_history[i-1],
                                                 states[i-1],
                                                 actions[i-1])
...
answer:
<state>
Summarize the observation of the current step. Include details such as accessibility
...
answer:
<next_state>
Predict the new state of the webpage after your proposed action is applied. 
Describe the effect that your proposed action will have on the content of the page
...
<response>
Observe your previous and current states of the browser environment. Classify
your behavior into one of three categories based on your progress towards the goal.
The categories are:
...
You should be extra-careful when assigning the "goal-reached" label. If you 
are unsure, please select "not-sure" instead.
</response>
"""
...
answer:
<think>
Observe your current state and proposed action in the browser environment, consider 
the previous states and actions, classify the proposed action into one of four categories 
...
"""
    ...
    def _parse_answer(self, text_answer): 
        ans_dict = {}
        ans_dict.update(parse_html_tags_raise(text_answer, optional_keys=["strategy"], merge_multiple=True))
        ans_dict.update(parse_html_tags_raise(text_answer, keys=["action"], merge_multiple=True))

        try:
            # just check if action can be mapped to python code but keep action as is
            # the environment will be responsible for mapping it to python
            self.action_space.action_space.to_python_code(ans_dict["action"])
        except Exception as e:
            raise ParseError(
                f"Error while parsing action\n: {e}\n"
                "Make sure your answer is restricted to the allowed actions."
    ...
    def _parse_state_answer(self, text_answer): 
        ans_dict = {}
        ans_dict.update(parse_html_tags_raise(text_answer, keys=["state"], merge_multiple=True))
        return ans_dict

    # def _parse_next_state_answer(self, text_answer): 
    #     ans_dict = {}
    #     ans_dict.update(parse_html_tags_raise(text_answer, keys=["next_state"], merge_multiple=True))
    #     return ans_dict

    def _parse_dynamics_answer(self, text_answer): 
        ans_dict = {}
        ans_dict.update(parse_html_tags_raise(text_answer, keys=["next_state", "status"], merge_multiple=True))
        return ans_dict

    def _parse_critic_answer(self, text_answer): 
        ans_dict = {}
        ans_dict.update(parse_html_tags_raise(text_answer, optional_keys=["think"], merge_multiple=True))
        ans_dict.update(parse_html_tags_raise(text_answer, keys=["response"], merge_multiple=True))
        return ans_dict
