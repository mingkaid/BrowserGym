...
class MyMainPrompt(PromptElement): 
    def __init__(self, 
        ...
        elif len(obs_history) == len(states) and len(states) == len(strategies) + 1 and len(strategies) == len(actions): 
            # strategy, use the obs and the state
            self.obs = self.get_obs_state(obs_history[-1], states[-1])
        elif len(obs_history) == len(states) and len(states) == len(strategies) and len(strategies) == len(actions) + 1: 
            # policy, use obs, state, and strategy
            self.obs = self.get_obs_state_strat(obs_history[-1], states[-1], active_strategy)
        elif len(obs_history) <= len(states) and len(states) == len(strategies) and len(strategies) >= len(actions) + 1:
...
"""
    ...
    def _parse_strategy_answer(self, text_answer): 
        ans_dict = {}
        ans_dict.update(parse_html_tags_raise(text_answer, keys=["strategy"], merge_multiple=True))
        return ans_dict
